...
class Bird:
	...
	def jump(self):
		self.vel = -10.5  # upwards negative velocity, downwards positive velocity
		self.tick_count = 0
		self.height = self.y
	...
	def draw(self, win):
		self.img_count += 1
		# controllp immagine da mostrare in base al counter
...
def draw_window(win, bird, pipes, base, score):
	win.blit(BG_IMG, (0, 0))  # draw on the window, topleft

	for pipe in pipes:
		pipe.draw(win)

	text = STAT_FONT.render("Score: " + str(score), 1, (255, 255, 255))
	win.blit(text, (WIN_WIDH - 10 - text.get_width(), 10))
...
def main():
	...
	base = Base(730)
	pipes = [Pipe(600)]
	win = pygame.display.set_mode((WIN_WIDH, WIN_HEIGHT))
